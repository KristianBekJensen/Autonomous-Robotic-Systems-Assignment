...
class MazeSolver(ScalarProblem):
    ...
    def __init__(self, maximize, visualization, timesteps , n_discrete_angles):
        self.visualize_evaluation = visualization
        self.n_discrete_angles = n_discrete_angles
        self.min_speed = 0
        self.max_speed = 3
        self.timesteps = timesteps
        super().__init__(maximize=maximize)

    def phenome_navigate(self, phenome, v_left, v_right, phi):
        

        strBits = real_to_binary(phi, self.n_discrete_angles, 0 , np.pi * 2)
        #print(f"String Bits {strBits}")
        ...
        return np.clip(v_left, self.min_speed, self.max_speed), np.clip(v_right, self.min_speed, self.max_speed)
    ...
    def evaluate(self, phenome):
        ...
        # Evaluation metrics 
        number_time_steps = 0
        max_time_steps = self.timesteps
        target_x, target_y = 700, 700
        average_path_deviation = 0 
        ...
        while running:
            ...
            if path:
                ...
                # Normalize to range [-π, π] first (this handles any value, even outside normal range)
                phi = ((phi + np.pi) % (2 * np.pi)) - np.pi
                
                # Take absolute value to get magnitude only (0 to π)
                phi = abs(phi)
                average_path_deviation += abs(phi)
                robot.v_left, robot.v_right = self.phenome_navigate(phenome, robot.v_left, robot.v_right, phi)

                # other possibility: calculate distance to the path 
                #average_path_deviation += distance_to_target([robot.x, robot.y], [path[4][0], path[4][1]])
            ...
            # phenom navigation only using the angle to estimate path 
            #robot.v_left, robot.v_right = self.phenome_navigate(phenome, robot.v_left, robot.v_right, phi)
        ...
        print("Episode has finished with avg path deviation: ", average_path_deviation / number_time_steps)

        # in this case right now no fitness function is needed
        #return fitness(average_path_deviation / number_time_steps)
        return average_path_deviation / number_time_steps
